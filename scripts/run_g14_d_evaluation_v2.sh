...
CONFIG="configs/g14_d_counterfactual_evaluation_v2.yaml"
OUTPUT="artifacts/g14_domain_generalization/counterfactual_evaluation_v2/evaluation.json"
...
case "$mode" in
  ...
  run)
    if [[ -s "$OUTPUT" ]]; then
      echo "Refusing to overwrite completed G14-D V2 evaluation: $OUTPUT" >&2
      exit 1
    fi
    "$PYTHON_BIN" -u -m dads_crnn.evaluate_g14_counterfactual \
      --config "$CONFIG" \
    ...
    ;;
  status)
    pgrep -af "dads_crnn.evaluate_g14_counterfactual.*g14_d_counterfactual_evaluation_v2.*--mode run" || true
    nvidia-smi --query-compute-apps=pid,used_memory --format=csv,noheader || true
    find artifacts/g14_domain_generalization/counterfactual_evaluation_v2 \
