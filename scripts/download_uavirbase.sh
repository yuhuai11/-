...
DEST_DIR="$ROOT/data/external_confirmation_v2/_raw/UaVirBASE"
FINAL="$DEST_DIR/Microphone_array.zip"
PART="$FINAL.part"
LOCK="$DEST_DIR/.download.lock"
URL="https://zenodo.org/records/15391924/files/Microphone_array.zip?download=1"
EXPECTED_MD5="3c4081511630f8045fda0347cac45534"

mkdir -p "$DEST_DIR"
exec 9>"$LOCK"
if ! flock -n 9; then
  echo "UaVirBASE download is already running; refusing a second writer." >&2
  exit 75
fi
...
if [[ -f "$FINAL" ]]; then
  if verify_file "$FINAL"; then
    echo "UaVirBASE is already complete and MD5-verified: $FINAL"
    exit 0
  fi
...
while true; do
  attempt=$((attempt + 1))
  echo "UaVirBASE curl attempt $attempt; existing partial bytes: $(stat -c %s "$PART" 2>/dev/null || echo 0)"
  ...
  if [[ -f "$PART" ]] && verify_file "$PART"; then
    mv "$PART" "$FINAL"
    echo "UaVirBASE download complete; MD5 verified: $EXPECTED_MD5"
