...
| DADS | 无人机/背景二分类音频 | G7训练、验证和内部测试 | 是，仅训练G7 |
...
| Test | 18,670 | 27,221 | 45,891 |
| 总计 | 125,314 | 179,692 | 305,006 |

每个样本为1秒、16 kHz波形。历史清单先在每个Parquet行所代表的`AudioRef`层面随机
分配Train、Validation和Test，再展开为1秒片段，因此同一个`AudioRef`的片段不会跨
分区；但历史流程没有按`source_path`或原始音频SHA256进行全局分组隔离。后续原始
音频SHA256审计发现15个重复录音、244个重复片段，其中6组重复哈希跨分区；去重后共
304,762个片段。需要严谨说明：当前G7正式权重是在历史清单上训练的，尚未在
dedup v2上完整重训。
...
G7 seed 42最佳权重来自第7轮。DADS内部Test结果：
...
45,891个Test片段中，FN为39、FP为125。G7在DADS内部已经接近饱和，继续只优化内部
F1的收益很有限。
...
只有546个平衡样本；它不能抵消Unseen和val_ood上的下降。当前G7主要问题已从内部
分类转为跨来源、低误报和概率迁移。
...
> 系统已实现高性能DADS内部无人机检测，并证明冻结G7表征能够支持九个已知型号的
> 录音级分类；但现有线性Outlier Exposure无法可靠推广到未参与开发的X6D/Y6，
> 因而尚未实现可部署的通用开放集型号识别。
...
7. 在DADS dedup v2上复现G7，确认去重后的非劣性。
